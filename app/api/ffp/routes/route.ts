...
export async function GET(request: NextRequest) {
  try {
    ...
    const primitives = Object.values(rows.reduce<Record<string, {
      id: string;
      primitive: string;
      executions: number;
      ...
      const existing = acc[row.primitive];
      if (existing) {
        existing.executions += 1;
        if (row.fallbackUsed) existing.fallbackCount += 1;
        return acc;
      }
      acc[row.primitive] = {
        id: row.primitive,
        ...
        latestTool: row.tool,
        status: row.status,
        fallbackCount: row.fallbackUsed ? 1 : 0,
      };
      return acc;
    }, {}));
    ...
    return NextResponse.json({
      enabled: false,
      consensusAvailable: false,
      message: 'FFP is disabled. Returned rows are compatibility records only, not active proof, validator, transaction, or consensus events.',
      routes: rows,
      primitives,
    });
  } catch (error: unknown) {
