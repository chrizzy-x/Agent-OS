...
interface Endpoint {
  method: 'GET' | 'POST';
  path: string;
  auth: 'None' | 'Bearer (Agent)' | 'Bearer (Ops Admin)' | 'Bearer (Cron/Admin)';
  desc: string;
...
const endpoints: Endpoint[] = [
  ...
  },
  {
    ...
    desc: 'Create an agent account and receive a 90-day bearer token.',
    body: [
  ...
  },
  {
    ...
    desc: 'Authenticate an existing agent account and receive a fresh 90-day bearer token.',
    body: [
  ...
  {
    method: 'POST', path: '/api/studio/command', auth: 'Bearer (Agent)',
  ...
  },
  {
    ...
    response: '{ "skills": [...], "pagination": { "page": 1, "limit": 20, "total": 42 } }',
  },
  {
  ...
  {
    method: 'POST', path: '/api/ops/crew/bootstrap', auth: 'Bearer (Ops Admin)',
    desc: 'Idempotently restore active and standby coverage across the canonical feature catalog.',
    response: '{ "success": true, "result": {...} }',
  },
  {
    method: 'POST', path: '/api/ops/crew/failover', auth: 'Bearer (Ops Admin)',
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-10 text-sm text-amber-800">
          <strong>Authentication:</strong> agent routes use a bearer token returned by <code className="font-mono bg-amber-100 px-1 rounded">/api/signup</code> or <code className="font-mono bg-amber-100 px-1 rounded">/api/signin</code>.
          Ops-control mutations require an ops-admin bearer token or the admin token. Cron execution requires the cron secret or admin token.
        </div>
        ...
        <div className="mt-12 border border-gray-200 rounded-xl overflow-hidden">
          ...
          <div className="px-5 py-4">
            <div className="space-y-2">
              {[
                { code: '400', msg: 'Bad Request - missing, invalid, or expired input' },
                { code: '401', msg: 'Unauthorized - missing or invalid bearer token' },
                { code: '403', msg: 'Forbidden - authenticated but missing required role' },
                { code: '404', msg: 'Not Found - requested route or resource does not exist' },
                { code: '409', msg: 'Conflict - duplicate resource such as an existing email or skill install' },
                { code: '500', msg: 'Internal Server Error - check the JSON error message for details' },
              ].map(error => (
                <div key={error.code} className="flex items-center gap-3 text-sm">
                  <code className="font-mono font-bold text-gray-900 w-8">{error.code}</code>
                  <span className="text-gray-600">{error.msg}</span>
                </div>
              ))}
            </div>
          </div>
