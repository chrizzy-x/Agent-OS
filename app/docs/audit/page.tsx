...
const buckets: AuditBucket[] = [
  {
    ...
    items: [
      ...
      'Versioned public surfaces now align on AgentOS V6.6.8 and the canonical production host.',
      ...
      <div className="max-w-5xl mx-auto px-4 py-12 space-y-8">
        <section>
          ...
          <p className="text-lg" style={{ color: 'var(--text-muted)' }}>
            This report reflects the V6.6.8 release pass for <code>{APP_URL}</code>. The legacy deployment host <code>{LEGACY_APP_URL}</code> remains compatibility-only.
          </p>
        </section>
        ...
        <section className="card p-6">
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            V6.6.8 is the runtime contract release on top of the Studio-first Super AgentOS surface: capability graph metadata, workspace context packages, task lifecycle fields, execution recovery, files, memory, apps, skills, workflows, MCP routing, and diagnostic failures remain connected through production APIs.
          </p>
        </section>
      </div>
