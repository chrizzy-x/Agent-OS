...
const GROUPS: Record<GroupKey, GroupDefinition> = {
  ...
  ffp: {
    name: 'FFP Temp Routing',
    badge: 'FFP temp',
    description: 'Temporary coordination routing for multi-agent work until real Fabric Furge Protocol ships.',
    competitor: 'CrewAI with app-level handoff logic bolted on afterward',
    standout: 'Agent OS exposes the routing point now without claiming consensus, validator voting, or proof results.',
    useCaseTemplates: [
      'A treasury operations crew uses {name} to route high-risk multi-agent work through one observable execution layer.',
      'A medical review workflow uses {name} to keep subagent collaboration auditable while real FFP remains temporary.',
    ],
  },
  mcp: {
    ...
    standout: 'Agent OS adds shared routing, policy checks, and FFP-temp-aware execution visibility on top of standard MCP access.',
    useCaseTemplates: [
...
const PLATFORM_FEATURES: CatalogSeed[] = [
  ...
  { id: 11, slug: 'validation', name: 'Validation', short: 'Validate agents before they participate in high-risk multi-agent work.', group: 'ffp', kind: 'platform_feature' },
...
export const PROJECT_DETAILS = {
  ...
  differentiators: [
    ...
    'MCP routing with FFP temp wiring and full logging',
